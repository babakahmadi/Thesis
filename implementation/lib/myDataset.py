...
def generateTwoGaussianNum(N):
    ...
    X[:N/2] = np.random.multivariate_normal(mean1, cov1, N/2)
    X[N/2:] = np.random.multivariate_normal(mean2, cov2, N/2)
    y[N/2:] = np.ones(N/2)
    return X,y,"Gaussian"
...
def generateConcentric():
    ...
    x = concentric['X'][:,0]
    y = concentric['X'][:,1]
    label = concentric['Y'][:,0]
    label = (label==1)
    return x,y,label,"Concentric"
